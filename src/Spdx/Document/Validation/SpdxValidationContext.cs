...
internal sealed class SpdxValidationContext
{
    ...
    {
        var error = new SpdxValidationError(message).WithInfoIfNotNullOrEmpty("Path", Path);
        _errors.Add(error);
        return error;
    }

    public SpdxValidationError AddError(string property, string message)
    {
        using (PushPath(property))
        {
            var error = new SpdxValidationError(message).WithInfoIfNotNullOrEmpty("Path", Path);
            _errors.Add(error);
            return error;
